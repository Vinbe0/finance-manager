...
if menu == "Overview":
    ...
    fig_bal = px.bar(x=accounts_names, y=balances, labels={"x": "Account", "y": "Balance (KZT)"}, title="Баланс по аккаунтам", template="plotly_dark")
    ...
    st.write(f"Прогноз расходов по категории {selected_name}: {result2:,} KZT")
    st.write(f"Без кэша: {uncached_time:.3f} ms, С кэшем: {cached_time:.3f} ms")
